...
class VisualGAT(nn.Module):

    def __init__(self, embed_size):
        super(VisualGAT, self).__init__()
        self.embed_size = embed_size

        self.GAT = GAT(in_channels=embed_size, inter_channels=embed_size)
    ...
    def forward(self, images):
        ...
        GAT_img_emd = images.permute(0, 2, 1)

        GAT_img_emd = self.GAT(GAT_img_emd)

        GAT_img_emd = GAT_img_emd.permute(0, 2, 1)

        v_global = torch.mean(GAT_img_emd, dim=1)

        GAT_img_emd = l2norm(GAT_img_emd)
        v_global = l2norm(v_global)

        return GAT_img_emd, v_global
    ...
    def load_state_dict(self, state_dict):
        ...
        for name, param in state_dict.items():
            if name in own_state:
                new_state[name] = param
        super(VisualGAT, self).load_state_dict(new_state)
...
class EncoderSimilarity(nn.Module):
    def __init__(self, embed_size, sim_dim, module_name='MHAtt', sgr_step=3):
        super(EncoderSimilarity, self).__init__()
        self.module_name = module_name

        self.v_gcn = VisualGAT(embed_size)
        ...
        if module_name == 'MHAtt':
            self.MHAttMFB_module = nn.ModuleList([MHAtt(sim_dim) for i in range(sgr_step)])
        else:
    ...
    def forward(self, img_emb, cap_emb, cap_lens):
        ...
        img_emb, img_glo = self.v_gcn(img_emb)
        ...
        for i in range(n_caption):
            ...
            if self.module_name == 'MHAtt':
                for module in self.MHAttMFB_module:
                    sim_emb = module(sim_emb, sim_emb, sim_emb, None)
                sim_vec = sim_emb[:, 0, :]
            else:
                raise ValueError('Invalid input of opt.module_name in opts.py')
